...
export default function ConsultantPaymentsPage() {
  ...
  const handleTopup = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? 'Top-up başlatılamadı.');
  ...
  const TXN_LABELS: Record<string, string> = {
    topup_stripe: 'Yükleme (Stripe)', topup_bank: 'Yükleme (Banka)',
    deduction_platform_fee: 'Platform Ücreti', refund: 'İade',
  };
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="rounded-2xl border border-[#00F0FF]/20 bg-[#00F0FF]/5 p-6">
        <p className="text-sm text-white/50">Ki Wallet Bakiyesi</p>
        <p className="mt-1 text-4xl font-bold text-[#00F0FF]">${(wallet / 100).toFixed(2)}</p>
        <p className="mt-1 text-xs text-white/30">
          Ödeme modu: <span className="text-white/60">{mode === 'own_keys' ? 'Kendi Stripe (Mode B)' : mode === 'ki_escrow' ? 'Ki Escrow (Mode A)' : mode === 'ki_connect' ? 'Stripe Connect (Mode C)' : mode}</span>
        </p>
      </div>
      ...
      {mode === 'own_keys' && (
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6 space-y-4">
          <h2 className="font-semibold text-white">Bakiye Yükle</h2>
          <p className="text-sm text-white/40">Her randevu için Platform ücreti (%10) cüzdanınızdan düşülür. Cüzdanınızda yeterli bakiye olmalıdır.</p>
          ...
          >
            {toppingUp ? 'Yönlendirilıyor…' : `$${(topupAmt / 100).toFixed(0)} Yükle`}
          </button>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
        ...
        {txns.length === 0 ? (
          <p className="py-10 text-center text-sm text-white/30">Henüz işlem yok.</p>
        ) : (
            ...
            {txns.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <p className="text-xs text-white/30">{new Date(tx.created_at).toLocaleString('tr-TR')}</p>
                </div>
